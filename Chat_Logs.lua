...
if pg:FindFirstChild("AccChat") then 
    pg.AccChat:Destroy() 
end
...
mb.MouseButton1Click:Connect(function() 
    ob.Position = mf.Position 
    mf.Visible = false 
    ob.Visible = true 
end)

ob.MouseButton1Click:Connect(function() 
    mf.Position = ob.Position 
    ob.Visible = false 
    mf.Visible = true 
end)
        ...
        for _, c in ipairs(channels:GetChildren()) do
            if c:IsA("TextChannel") then
                c.MessageReceived:Connect(function(m) 
                    if m.TextSource and m.Text ~= "" then 
                        add(m.TextSource.Name, m.Text) 
                    end 
                end)
            end
        end
    ...
    if rm then
        ...
        if om and om:IsA("RemoteEvent") then
            om.OnClientEvent:Connect(function(d)
                if d and d.FromPlayer and d.Message then 
                    add(d.FromPlayer.Name or tostring(d.FromPlayer), tostring(d.Message)) 
                end
            end)
        end
    end
