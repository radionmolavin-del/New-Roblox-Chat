...
mf.Position = UDim2.new(0.5, -155, 0.4, -90);
...
auth.Position = UDim2.new(1, -150, 0, 0);
...
mb.Position = UDim2.new(1, -44, 0, 3);
mb.BackgroundTransparency = 1;
mb.Text = "_";
mb.TextColor3 = Color3.fromRGB(150, 150, 150);
mb.TextSize = 14;
...
ob.Size = UDim2.new(0, 32, 0, 32);
ob.Position = UDim2.new(0.5, -16, 0.4, -16);
...
tb.Size = UDim2.new(1, -85, 0, 26);
tb.Position = UDim2.new(0, 8, 0, 146);
...
nc.Parent = nb;
mb.MouseButton1Click:Connect(function() ob.Position = mf.Position; mf.Visible = false; ob.Visible = true end);
ob.MouseButton1Click:Connect(function() mf.Position = ob.Position; ob.Visible = false; mf.Visible = true end);
...
local text = tb.Text;
if text ~= "" then send(text); tb.Text = "" end
end;
sb.MouseButton1Click:Connect(onSend);
tb.FocusLost:Connect(function(ep) if ep then onSend() end end);
if TCS.ChatVersion == Enum.ChatVersion.TextChatService then
local channels = TCS:FindFirstChild("TextChannels");
if channels then
for _, c in ipairs(channels:GetChildren()) do
if c:IsA("TextChannel") then
c.MessageReceived:Connect(function(m) if m.TextSource and m.Text ~= "" then add(m.TextSource.Name, m.Text) end end)
end
end;
...
if d and d.FromPlayer and d.Message then add(d.FromPlayer.Name or tostring(d.FromPlayer), tostring(d.Message)) end
end)
end
end
end;
add("System", "Welcome to the chat!");
